...
indices = lavatar.generate_indices() # False to only extract timestamp & landmarks indices
frames = []
...
def extract_indices( l ):
	...
	for word in l.split( FIELD_SEPARATOR ):
		...
		if word == TIMESTAMP_FIELD:
			...
			if 'gaze_0' in indices.keys() and 'gaze_1' in indices.keys():
				for xyz in GAZE_FIELDS:
					for axis in xyz:
						if axis == word:
							field = word[:6]
							if indices[field] == None:
								indices[field] = [None,None,None]
							ax = word[7:8]
							if ax == 'x':
								indices[field][0] = i
							elif ax == 'y':
...
def load_csv():
	...
	for line in raw_file:
		...
		if not indices['valid']:
			...
			f = lavatar.parse_text_frame( line.split( FIELD_SEPARATOR ), indices )
			if not f['valid']:
				continue
			frames.append(  )
