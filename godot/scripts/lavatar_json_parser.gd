...
export (bool) var ANIMATION_enabled = true
export (bool) var MAPPING_enabled = true
export (bool) var POSE_EULER_enabled = true
export (bool) var OSC_enabled = false
...
var pose_euler_damped = {}

var mapping = {
	
	'neck01': { 'pose_euler': [0.2, 0.05] },
	'neck02': { 'pose_euler': [0.3, 0.2] },
	'head': { 'pose_euler': [0.6, 0.4] },
...
func update_correction( keys, trans, rot, sca ):
	...
	for k in keys:
		if k.find( "left" ) != -1:
			...
			corrections[k]['correction'] = Transform( Basis(q).scaled( sca ), Vector3() )
			corrections[k]['correction'].origin = trans * Vector3(-1,1,1)
		else:
			q.set_euler( rot )
			corrections[k]['correction'] = Transform( Basis(q).scaled( sca ), Vector3() )
			corrections[k]['correction'].origin = trans
	...
	if b:
		...
		var jdata = {
			...
			'corrections': {
				...
				'brow_rotation': brow_rotation,
				'brow_translation': brow_translation,
				'brow_scale': brow_scale,
				
				'eye_rotation': eye_rotation,
				'eye_translation': eye_translation,
				'eye_scale': eye_scale,
				
				'mouth_rotation': mouth_rotation,
				'mouth_translation': mouth_translation,
				'mouth_scale': mouth_scale,
				
				'nose_rotation': nose_rotation,
				'nose_translation': nose_translation,
				'nose_scale': nose_scale,
				
				'nostril_rotation': nostril_rotation,
				'nostril_translation': nostril_translation,
				'nostril_scale': nostril_scale
...
func set_load_config( b ):
	...
	if b:
		...
		if result_json.error == OK:  # If parse OK
			...
			var corrections = jdata['corrections']
			var ln = get_node( lavatar_node_path )
			
			anchor_rotation = ln.deserialise_vec3( corrections['anchor_rotation'] )
			anchor_translation = ln.deserialise_vec3( corrections['anchor_translation'] )
			anchor_scale = ln.deserialise_vec3( corrections['anchor_scale'] )
			
			brow_rotation = ln.deserialise_vec3( corrections['brow_rotation'] )
			brow_translation = ln.deserialise_vec3( corrections['brow_translation'] )
			brow_scale = ln.deserialise_vec3( corrections['brow_scale'] )
			
			eye_rotation = ln.deserialise_vec3( corrections['eye_rotation'] )
			eye_translation = ln.deserialise_vec3( corrections['eye_translation'] )
			eye_scale = ln.deserialise_vec3( corrections['eye_scale'] )
			
			mouth_rotation = ln.deserialise_vec3( corrections['mouth_rotation'] )
			mouth_translation = ln.deserialise_vec3( corrections['mouth_translation'] )
			mouth_scale = ln.deserialise_vec3( corrections['mouth_scale'] )
			
			nose_rotation = ln.deserialise_vec3( corrections['nose_rotation'] )
			nose_translation = ln.deserialise_vec3( corrections['nose_translation'] )
			nose_scale = ln.deserialise_vec3( corrections['nose_scale'] )
			
			nostril_rotation = ln.deserialise_vec3( corrections['nostril_rotation'] )
			nostril_translation = ln.deserialise_vec3( corrections['nostril_translation'] )
			nostril_scale = ln.deserialise_vec3( corrections['nostril_scale'] )
			
			mapping = jdata['mapping']
			ln.deserialise( jdata['skeleton'] )
			
			_brow_rotation( brow_rotation )
			_eye_rotation( eye_rotation )
			_mouth_rotation( mouth_rotation )
			_nose_rotation( nose_rotation )
			_nostril_rotation( nostril_rotation )
			anchor_correction()
...
func load_sound():
	...
	if not 'sound' in animation:
		return
	if not 'path' in animation['sound'] or animation['sound']['path'] == null:
...
func animation_duration():
	return animation['frames'][len(animation['frames'])-1]['timestamp']

func postprocess_frame( frame, prev_frame, pc ):
...
func load_frame():
	...
	postprocess_frame( frame, prev_frame, pc )
...
func apply_mapping( k ):
	...
	if 'pose_euler' in lconf and POSE_EULER_enabled:
		
		if lconf['pose_euler'][1] != 1:
			if not k in pose_euler_damped:
				pose_euler_damped[k] = Vector3()
			pose_euler_damped[k] = interpolate_euler( pose_euler_damped[k], get_pose_euler() * lconf['pose_euler'][0], lconf['pose_euler'][1] )
			lavatar_node.bone_rotate( lbone, pose_euler_damped[k] )
		else:
			lavatar_node.bone_rotate( lbone, get_pose_euler() * lconf['pose_euler'][0] )
		return
...
func load_osc_frame( msg ):
	
	var prev_frame = current_frame.duplicate(true)
	
	# unpacking
	var i = 0
	var id = msg.arg(i)
	i+=1
	current_frame['timestamp'] = msg.arg(i)
	i+=1 + 1
	for j in range(0, 68*3, 3):
		current_frame['landmarks'][int(j/3)] = Vector3( msg.arg(i+j), msg.arg(i+j+1), msg.arg(i+j+2) )
	i += 68*3 + 1
	for j in range(0, 6, 3):
		current_frame['gazes'][int(j/3)] = Vector3( msg.arg(i+j), msg.arg(i+j+1), msg.arg(i+j+2) )
	i += 6 + 1
	current_frame['pose_euler'] = Vector3( msg.arg(i), msg.arg(i+1), msg.arg(i+2) )
	i += 3 + 1
	current_frame['aabb']['center'] = Vector3( msg.arg(i), msg.arg(i+1), msg.arg(i+2) )
	current_frame['aabb']['center'] = Vector3()
	i += 3 + 1
	current_frame['aabb']['min'] = Vector3( msg.arg(i), msg.arg(i+1), msg.arg(i+2) )
	i += 3 + 1
	current_frame['aabb']['max'] = Vector3( msg.arg(i), msg.arg(i+1), msg.arg(i+2) )
	i += 3 + 1
	current_frame['aabb']['size'] = Vector3( msg.arg(i), msg.arg(i+1), msg.arg(i+2) )
	...
	postprocess_frame( current_frame, prev_frame, 1 )
...
#
#	for i in range( animation['landmark_count'] ):
#		current_frame['landmarks'][i] = pose.xform( current_frame['landmarks'][i] )
#		$landmarks.get_child( i ).translation = current_frame['landmarks'][i]
...
#		var upper_v3 = Vector3()
#		for id in upper['indices']:
#			upper_v3 += current_frame['gazes'][ id ]
#		upper_v3 /= len( upper['indices'] )
#		# average lower
#		var lower_v3 = Vector3()
#		for id in lower['indices']:
#			lower_v3 += current_frame['gazes'][ id ]
#		lower_v3 /= len( lower['indices'] )
#		var pos = global_transform.xform(  ( upper_v3 + lower_v3 ) * 0.5 )
#		current_frame['gazes'][i] = current_frame['gazes'][i].normalized()
#		var la = pos + current_frame['gazes'][i]
#		$gazes.get_child( i ).look_at_from_position( pos, pos + current_frame['gazes'][i], Vector3(0,1,0) )
...
func _process(delta):
	...
	if OSC_enabled:
		if osc_in == null:
			osc_in = get_node("osc_in")
			osc_in.start()
		while osc_in.has_waiting_messages():
			var msg = osc_in.get_next_message()
			var addr = msg.address()
			if addr == '/of/ff':
				load_osc_frame( msg )
	
	if not lavatar_node.debug:
		if MAPPING_enabled:
			for k in mapping:
				apply_mapping( k )
